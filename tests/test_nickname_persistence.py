...
def test_identity_service_updates_user_nickname(tmp_path) -> None:
    ...
    service.ensure_user_and_session(ConversationScope.private(platform="test", user_id="u1"), nickname="QQ昵称A")
    user = user_repo.get("u1")
    assert user is not None
    assert user.nickname == "QQ昵称A"

    # 不传 nickname / 传空字符串，不应覆盖已存在昵称
    service.ensure_user_and_session(ConversationScope.private(platform="test", user_id="u1"))
    user2 = user_repo.get("u1")
    assert user2 is not None
    assert user2.nickname == "QQ昵称A"

    service.ensure_user_and_session(ConversationScope.private(platform="test", user_id="u1"), nickname="   ")
    user3 = user_repo.get("u1")
    assert user3 is not None
    assert user3.nickname == "QQ昵称A"

    # 传入新 nickname 应更新
    service.ensure_user_and_session(ConversationScope.private(platform="test", user_id="u1"), nickname="QQ昵称B")
